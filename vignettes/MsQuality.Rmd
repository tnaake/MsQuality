...
The `MsQuality` package is built upon the `Spectra` package.
...
## Create `Spectra` object from (feature-extracted) intensity tables
...
each samples (objects `sps_c_rplc` and `sps_c_hilic`).
...
The final data set contains `r length(unique(dataOrigin(sps_c_rplc)))`
paired samples (i.e. `r length(unique(dataOrigin(sps_c_rplc)))` samples derived 
from RPLC and `r length(unique(dataOrigin(sps_c_rplc)))` samples derived 
from HILIC).

# Calculating the quality metrics on `Spectra` objects

Currently, the following metrics are included:
```{r}
qualityMetrics(sps_c_rplc)
```
...
a `Spectra` object as input, a character vector of metrics
...
dO_unique <- unique(dataOrigin(sps_c_rplc))[1]
sps_rplc_subset <- sps_c_rplc[dataOrigin(sps_c_rplc) %in% dO_unique, ]
sps_hilic_subset <- sps_c_hilic[dataOrigin(sps_c_hilic) %in% dO_unique, ]

## RPLC
metrics_rplc_sps_Q1 <- calculateMetrics(object = sps_rplc_subset,
    metrics = qualityMetrics(sps_rplc_subset),
    relativeTo = "Q1", msLevel = 1L)
metrics_rplc_sps_Q1
metrics_rplc_sps_previous <- calculateMetrics(object = sps_rplc_subset,
    metrics = qualityMetrics(sps_rplc_subset),
    relativeTo = "previous", msLevel = 1L)
metrics_rplc_sps_previous
```

Alternatively, a `Spectra` object might be passed to the function
`calculateMetricsSpectra`. The function will, similarly as in `calculateMetrics`
iterate over the samples (referring to entries in `dataOrigin(sps))`) and 
calculate the quality metrics on the corresponding `Spectra`s.

There are in total `r length(unique(dataOrigin(sps_c_rplc)))` samples 
respectively in the objects `sps_c_rplc` and `sps_c_hilic`. To improve 
...
## define metrics
metrics_sps <- c("rtDuration", "rtOverTicQuantile", "rtOverMsQuarters",
    "ticQuantileToQuantileLogRatio", "numberSpectra", "medianPrecursorMz",                
    "rtIqr", "rtIqrRate", "areaUnderTic")

## subset the Spectra objects
dO_unique <- unique(dataOrigin(sps_c_rplc))[1:20]
sps_rplc_subset <- sps_c_rplc[dataOrigin(sps_c_rplc) %in% dO_unique, ]
sps_hilic_subset <- sps_c_hilic[dataOrigin(sps_c_hilic) %in% dO_unique, ]

## for RPLC-derived Spectra
metrics_rplc_sps <- calculateMetricsFromSpectra(spectra = sps_rplc_subset,
    metrics = metrics_sps, relativeTo = "Q1", msLevel = 1L)

## for HILIC-derived Spectra
metrics_hilic_sps <- calculateMetricsFromSpectra(spectra = sps_hilic_subset,
    metrics = metrics_sps, relativeTo = "Q1", msLevel = 1L)
```

When passing an `Spectra` object to `calculateMetrics` a `data.frame`
object is returned with the samples (derived from the rownames of 
`dataOrigin(sps)`) in the rows and the metrics in columns. 

We will show here the objects `metrics_rplc_sps` and `metrics_hilic_sps`
```{r paged_table_metrics, eval = TRUE, echo = FALSE}
print("metrics_rplc_sps")
rmarkdown::paged_table(as.data.frame(metrics_rplc_sps))
print("metrics_hilic_sps")
rmarkdown::paged_table(as.data.frame(metrics_hilic_sps))
```
...
```{r}
metrics_sps <- rbind(metrics_rplc_sps, metrics_hilic_sps)
plotMetric(qc = metrics_sps, metric = "numberSpectra")
```
...
```{r}
plotMetric(qc = metrics_sps, metric = "ticQuantileToQuantileLogRatio")
```

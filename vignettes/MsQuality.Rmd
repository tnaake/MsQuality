...
each samples (objects `sps_rplc` and `sps_hilic`).
...
- *ticQuantileToQuantileLogRatio*, **MS1 TIC-change quartile ratios** 
...
- *ticQuantileToQuantileLogRatio*, **MS1 TIC quartile ratios** (MS:4000058),
...
of scan events or the TICs directly. One `Spectra` object may also contain
...
`ticQUantileToQuantileLogRatio` is run with the parameter combinations 
`relativeTo = "Q1", mode = "TIC", msLevel = c(1L, 2L)`.
...
There are in total `r length(unique(sps_rplc$dataOrigin))` samples 
respectively in the objects `sps_rplc` and `sps_hilic`. To improve 
...
## subset the Spectra objects
sps_subset <- sps_comb[grep(paste0("Sample.", 1:20, "_"), sps_comb$dataOrigin), ]

## for RPLC-derived and HILIC-derived Spectra
metrics_sps <- calculateMetricsFromSpectra(spectra = sps_subset,
...
We will show here the objects `metrics_sps`
```{r paged_table_metrics, eval = TRUE, echo = FALSE}
print("metrics_sps")
rmarkdown::paged_table(as.data.frame(metrics_sps))
```

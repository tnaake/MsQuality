...
The final data set contains `r length(unique(sps_c_rplc$dataOrigin))`
paired samples (i.e. `r length(unique(sps_c_rplc$dataOrigin))` samples derived 
from RPLC and `r length(unique(sps_c_rplc$dataOrigin))` samples derived 
from HILIC).
...
dO_unique <- unique(sps_c_rplc$dataOrigin)[1]
sps_rplc_subset <- sps_c_rplc[sps_c_rplc$dataOrigin %in% dO_unique, ]
sps_hilic_subset <- sps_c_hilic[sps_c_hilic$dataOrigin %in% dO_unique, ]
...
iterate over the samples (referring to entries in `sps$dataOrigin`) and 
calculate the quality metrics on the corresponding `Spectra`s.

There are in total `r length(unique(sps_c_rplc$dataOrigin))` samples 
...
## subset the Spectra objects
dO_unique <- unique(sps_c_rplc$dataOrigin)[1:20]
sps_rplc_subset <- sps_c_rplc[sps_c_rplc$dataOrigin %in% dO_unique, ]
sps_hilic_subset <- sps_c_hilic[sps_c_hilic$dataOrigin %in% dO_unique, ]
...
`sps$dataOrigin`) in the rows and the metrics in columns. 

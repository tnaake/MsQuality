...
objects if one of the samples only contains missing values. 

```{r remove_NA, warning = FALSE, message = TRUE}
inds_remove <- lapply(seq_along(sps_l_rplc), 
      function(x) is.na(sps_l_rplc[[x]]) | is.na(sps_l_hilic[[x]]))
...
rmarkdown::paged_table(metrics_rplc_msexp)
print("metrics_hilic_msexp")
rmarkdown::paged_table(metrics_hilic_msexp)
```
...
```{r}
library("reshape2")
metrics_rplc_l <- melt(metrics_rplc_msexp)
metrics_hilic_l <- melt(metrics_hilic_msexp)

## add information on the separation technology
metrics_rplc_l <- cbind(metrics_rplc_l, method = "RPCL")
metrics_hilic_l <- cbind(metrics_hilic_l, method = "HILIC")
...
```{r}
library("ggplot2")
ggplot(subset(metrics_l, Var2 == "numberSpectra")) +
  geom_point(aes(x = Var1, y = value, col = method)) +
...
```{r}
ggplot(subset(metrics_l, grepl(Var2, pattern = "areaUnderTICRTquantiles"))) +

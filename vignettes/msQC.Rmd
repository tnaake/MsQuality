...
to `Spectra` objects. The sheets `Methods` and `Raw data` are stored 
as txt files in `msQC`. 

We will create per sample one `Spectra` object. The data set of 
@Lee2019 contains samples in columns and 
...
path_Lee2019 <- system.file("data/", package = "msQC")
path_to_meta <- paste(path_Lee2019, 
                        "Lee_et_al_2019_Stables_rev1_Methods.txt", sep = "/")
meta <- read.table(path_to_meta, sep = "\t", dec = ".", header = TRUE)
## print number of metabolites per measurement (meta data)
table(meta$Method)

path_to_vals <- paste(path_Lee2019,
                        "Lee_et_al_2019_Stables_rev1_Raw_data.txt", sep = "/")
vals <- read.table(path_to_vals, sep = "\t", dec = ".", header = TRUE)

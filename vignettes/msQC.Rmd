...
store the mass spectral data of multiple samples. `msQC` enables the user
...
MS$^3$ features. If the user specifies 
`params = list(relativeTo = c("Q1", "previous"), MSLevel = c(1L, 2L))`, 
`ticQUantileToQuantileLogRatio` is run 
with the parameter combinations `relativeTo = "Q1", MSLevel = 1L`, 
...
metrics_rplc_sps <- calculateMetrics(object = sps_rplc_sample_1,
    ...
    params = list(relativeTo = c("Q1", "previous"), MSLevel = 1L))
metrics_rplc_sps
...
metrics_rplc_l <- metrics_rplc_msexp |> 
    ...
    as_tibble() |> 
    pivot_longer(cols = 2:(ncol(metrics_rplc_msexp) + 1), names_to = "metric")
metrics_hilic_l <- metrics_hilic_msexp |> 
    as.data.frame() |>
    rownames_to_column(var = "sample") |>
    as_tibble() |> 
    pivot_longer(cols = 2:(ncol(metrics_hilic_msexp) + 1), names_to = "metric")
...
```{r}
library("ggplot2")
ggplot(subset(metrics_l, metric == "numberSpectra_MSLevel1")) +
